        ...
        try {
            try {
                ...
                try {
                    ...
                    if (cursorQuery.moveToFirst()) {
                        do {
                            arrayList.add(new zzab(this, zzaY(cursorQuery.getString(2)), cursorQuery.getLong(1), zzam.zzbn(cursorQuery.getString(3)), cursorQuery.getLong(0), cursorQuery.getInt(4)));
                        } while (cursorQuery.moveToNext());
                    }
                    if (cursorQuery != null) {
                        cursorQuery.close();
    ...
    */
    public List<zzh> zzr(long j) throws Throwable {
        ...
        try {
            try {
                int iZzjv = zzhR().zzjv();
                Cursor cursorQuery = writableDatabase.query("properties", new String[]{"cid", "tid", "adid", "hits_count", "params"}, "app_uid=?", new String[]{String.valueOf(j)}, null, null, null, String.valueOf(iZzjv));
                try {
                    ArrayList arrayList = new ArrayList();
                    if (cursorQuery.moveToFirst()) {
                        do {
                            ...
                            Map<String, String> mapZzaZ = zzaZ(cursorQuery.getString(4));
                            if (TextUtils.isEmpty(string) || TextUtils.isEmpty(string2)) {
                                zzc("Read property with empty client id or tracker id", string, string2);
                            } else {
                                arrayList.add(new zzh(j, string, string2, z, j2, mapZzaZ));
                            }
                        } while (cursorQuery.moveToNext());
                    }
                    if (arrayList.size() >= iZzjv) {
