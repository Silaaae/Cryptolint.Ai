...
public class MediaRouteControllerDialog extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(C0188R.layout.mr_media_route_controller_material_dialog_b);
        ClickListener listener = new ClickListener();
        this.mDisconnectButton = (Button) findViewById(C0188R.id.disconnect);
        this.mDisconnectButton.setOnClickListener(listener);
        this.mStopCastingButton = (Button) findViewById(C0188R.id.stop);
        this.mStopCastingButton.setOnClickListener(listener);
        this.mSettingsButton = (ImageButton) findViewById(C0188R.id.settings);
        this.mSettingsButton.setOnClickListener(listener);
        this.mArtView = (ImageView) findViewById(C0188R.id.art);
        this.mTitleView = (TextView) findViewById(C0188R.id.title);
        this.mSubtitleView = (TextView) findViewById(C0188R.id.subtitle);
        this.mTitlesWrapper = findViewById(C0188R.id.text_wrapper);
        this.mPlayPauseButton = (ImageButton) findViewById(C0188R.id.play_pause);
        this.mPlayPauseButton.setOnClickListener(listener);
        this.mRouteNameView = (TextView) findViewById(C0188R.id.route_name);
        this.mCreated = true;
        if (update()) {
            this.mControlView = onCreateMediaControlView(savedInstanceState);
            FrameLayout controlFrame = (FrameLayout) findViewById(C0188R.id.media_route_control_frame);
            if (this.mControlView != null) {
                controlFrame.findViewById(C0188R.id.default_control_frame).setVisibility(8);
        ...
        if (this.mControlView == null) {
            ...
            if (this.mState != null) {
                ...
                if (isPlaying && supportsPause) {
                    ...
                    this.mPlayPauseButton.setImageResource(MediaRouterThemeHelper.getThemeResource(getContext(), C0188R.attr.mediaRoutePauseDrawable));
                    this.mPlayPauseButton.setContentDescription(getContext().getResources().getText(C0188R.string.mr_media_route_controller_pause));
                } else if (!isPlaying && supportsPlay) {
                    this.mPlayPauseButton.setVisibility(0);
                    this.mPlayPauseButton.setImageResource(MediaRouterThemeHelper.getThemeResource(getContext(), C0188R.attr.mediaRoutePlayDrawable));
                    this.mPlayPauseButton.setContentDescription(getContext().getResources().getText(C0188R.string.mr_media_route_controller_play));
                } else {
    ...
    private Drawable getIconDrawable() {
        if (this.mRoute.isConnecting()) {
            if (this.mMediaRouteConnectingDrawable == null) {
                this.mMediaRouteConnectingDrawable = MediaRouterThemeHelper.getThemeDrawable(getContext(), C0188R.attr.mediaRouteConnectingDrawable);
            }
            return this.mMediaRouteConnectingDrawable;
        }
        if (this.mMediaRouteOnDrawable == null) {
            this.mMediaRouteOnDrawable = MediaRouterThemeHelper.getThemeDrawable(getContext(), C0188R.attr.mediaRouteOnDrawable);
        }
    ...
    private final class ClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            IntentSender is;
            int id = v.getId();
            if (id == C0188R.id.stop || id == C0188R.id.disconnect) {
                if (MediaRouteControllerDialog.this.mRoute.isSelected()) {
                    MediaRouteControllerDialog.this.mRouter.unselect(id == C0188R.id.stop ? 2 : 1);
                }
                MediaRouteControllerDialog.this.dismiss();
                return;
            }
            if (id == C0188R.id.play_pause) {
            ...
            if (id == C0188R.id.settings && (is = MediaRouteControllerDialog.this.mRoute.getSettingsIntent()) != null) {
                try {
